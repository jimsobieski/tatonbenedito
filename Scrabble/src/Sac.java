...
public class Sac {
    ...
    public ArrayList<Lettre> changerLettres(ArrayList<Lettre> l) {
        ArrayList<Lettre> newLettres = new ArrayList<Lettre>();
        ajouterLettres(l);
        newLettres.addAll(piocher(l.size()));
        return newLettres;
    }
        ...
        while (cmpLettresAdd < nbLettres) {
            for (int i = 0; i < nbLettres; i++) {
                ...
                if (tabChoix.contains(nbrand)) {
                    ...
                    if (lesLettres.get(nbrand) != null) {
                        l.add(lesLettres.get(nbrand));
                        lesLettres.remove(nbrand);
                        tabChoix.add(nbrand);
                        cmpLettresAdd++;
                    }
